...
<body>
    ...
    @endif
        ...
        <th>
            <a href="{{ route('movies.create') }}" class="btn btn-primary mb-3">Thêm mới</a>
        </th>
                ...
                @foreach ($movies as $movie)
                    <tr>
                        ...
                        <td class="d-flex gap-1" >
                            <a href="{{ route('movies.edit', $movie->id) }}" class="btn btn-success">Sửa</a>
                            <form action="{{ route('movies.destroy', $movie->id) }}" method="post">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-danger" onclick="return confirm('Bạn có muốn xóa không?')">Xóa</button>
                            </form>
                            <a href="{{ route('movies.show', $movie->id) }}" class="btn btn-warning text-white">Chi tiết</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
